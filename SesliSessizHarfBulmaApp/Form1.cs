         ...
         /*   char[] sesliHarfler = new char[8] { 'a', 'e', 'ı', 'i', 'o', 'ö', 'u', 'ü' };
            ...
            if (rbSesli.Checked)
            {
                ...
                foreach (char kelimeKarakter in tbKelime.Text.ToLower())
                {
                    foreach (char sesliHarf in sesliHarfler)
                    {
                        ...
                        {
                            if (!sonuc.Contains(sesliHarf)) { sonuc += sesliHarf; break; }
            ...
            else
            {
                ...
                foreach (char kelimeKarakter in tbKelime.Text.ToLower())
                {
                    ...
                    foreach (char sesliHarf in sesliHarfler)
                    {
                        if (kelimeKarakter == sesliHarf) { sessizHarfMi = false; break; }
                    }
         ...
         /*   char[] sesliHarfler = new char[8] { 'a', 'e', 'ı', 'i', 'o', 'ö', 'u', 'ü' };
            ...
            foreach (char kelimeKarakter in tbKelime.Text.ToLower())
            {
                foreach (char sesliHarf in sesliHarfler)
                {
                    if (kelimeKarakter == sesliHarf)
                    {
                        if (!sesliler.Contains(sesliHarf)) { sesliler += sesliHarf; break; }
                    }
                }
            }
            ...
            if (rbSesli.Checked) sonuc = sesliler;
            else sonuc = sessizler;
            lSonuc.Text = sonuc; */ 
